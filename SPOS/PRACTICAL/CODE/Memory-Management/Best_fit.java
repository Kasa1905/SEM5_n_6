...
// Reads number of blocks and their sizes, then number of jobs and their sizes.
// Allocates each job to the block that leaves the smallest leftover (best fit),
// allowing multiple jobs per block until the block is full. Prints assignments
// and efficiency = sum(assigned job sizes) / sum(block sizes).
class Best_fit {
	public static void main(String[] args) {
		try (Scanner sc = new Scanner(System.in)) {
			...
			// Best Fit: among blocks that can fit, pick the one with smallest leftover
			for (int i = 0; i < m; i++) {
